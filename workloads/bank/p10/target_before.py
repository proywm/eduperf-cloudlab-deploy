'''Extracted kernel: Relation.selection (before optimization).

Only the selection method plus the minimal in-file support data types
it needs are kept. autocast/rstring/Header reconstructed as pure data
types (no injected cost) so the kernel runs on real input data.
'''
